...
module.exports = async (req, res) => {
  ...
  // lets the browser / Office viewers open pdf, docx, xlsx, pptx files
  // correctly straight from the URL.
  const safeName = filename.replace(/[^a-zA-Z0-9._-]/g, '_');
  const publicId = `${folder}/${Date.now()}_${safeName}`;

  // Cloudinary treats PDFs as an "image" asset (it can rasterize pages),
  // and delivering them that way is what makes them render inline in a
  // browser <embed> instead of forcing a generic download prompt. Every
  // other type (docx/xlsx/pptx/etc.) is uploaded as 'auto', same as before.
  const fileExt = (filename.split('.').pop() || '').toLowerCase();
  const uploadResourceType = fileExt === 'pdf' ? 'image' : 'auto';
  ...
  try {
    ...
    const cldRes = await fetch(`https://api.cloudinary.com/v1_1/${cloudName}/${uploadResourceType}/upload`, {
      method: 'POST',
    ...
    if (!cldRes.ok || data.error) {
      res.status(502).json({ error: (data && data.error && data.error.message) || 'Cloudinary upload failed' });
      return;
    }
